---
title: "NN Clustering on RD7 Essential Data"
...
## input files
FILE_SE_WUI="tbl/df_wui_rd7_essential_ui10.Rds"

## output files
FILE_OUT_TARGETS_RAW="tbl/df_target_raw_nnclusters_rd7_essential_ui10.csv"
FILE_OUT_TARGETS="tbl/df_target_nnclusters_rd7_essential_ui10.csv"
FILE_OUT_GENES="tbl/df_gene_nnclusters_rd7_essential_ui10.csv"
FILE_OUT_FINAL="img/heatmap-rd7-essential-nnclusters-labeled.pdf"

RDS_DWUI=sprintf("data/dwui/%s-dwui-target-gene-rd7-essential.Rds", format(Sys.time(), '%Y%m%d'))
RDS_ZDWUI=sprintf("data/dwui/%s-zdwui-target-gene-rd7-essential.Rds", format(Sys.time(), '%Y%m%d'))
...
df_dwui <- df_wui %>%
    filter(target_gene != 'non-targeting',
           (n_cells >= MIN_CELLS || target_gene %in% c("CPSF6", "FIP1L1", "NUDT21"))) %>%
...
It appears clusters 4 and 10 represent sets of perturbations with weak patterning.
Cluster 2 looks to have lots of heterogeneity in PC4.
...
#### Cluster 4
```{r plt_noise_target_cluster4_zdwui, fig.width=8, fig.height=8}
idx_cluster <- clusters_targets_raw %>% `[`(. == 4) %>% names()
...
#### Cluster 10
```{r plt_noise_target_cluster10_zdwui, fig.width=8, fig.height=8}
idx_cluster <- clusters_targets_raw %>% `[`(. == 10) %>% names()
pheatmap(t(zdwui_gene_target[,idx_cluster]), 
         color=COLORS_YKM,
         breaks=breaks_zdwui,
         fontsize_col=1, fontsize_row=1,
...
#### Cluster 2
```{r plt_noise_target_cluster2_zdwui, fig.width=8, fig.height=8}
idx_cluster <- clusters_targets_raw %>% `[`(. == 2) %>% names()
pheatmap(t(zdwui_gene_target[,idx_cluster]), 
         color=COLORS_YKM,
         breaks=breaks_zdwui,
         fontsize_col=1, fontsize_row=1,
...
Cluster 2 is patterned, but 4 and 10 can be excluded.


### All Other Clusters

```{r plt_signal_target_clusters_zdwui, fig.width=8, fig.height=8}
cluster_null_target <- c(4,10)
...
#### Cluster 2
```{r plt_noise_gene_cluster2_zdwui, fig.width=8, fig.height=8}
idx_cluster <- clusters_genes %>% `[`(. == 2) %>% names()
...
#### Cluster 5
```{r plt_noise_gene_cluster5_zdwui, fig.width=8, fig.height=8}
idx_cluster <- clusters_genes %>% `[`(. == 5) %>% names()
pheatmap(t(zdwui_gene_target[idx_cluster, idx_signal_targets]), 
         color=COLORS_YKM,
         breaks=breaks_zdwui,
         annotation_row=df_row_annots,
...
#### Cluster 16
```{r plt_noise_gene_cluster16_zdwui, fig.width=8, fig.height=8}
idx_cluster <- clusters_genes %>% `[`(. == 16) %>% names()
pheatmap(t(zdwui_gene_target[idx_cluster, idx_signal_targets]), 
         color=COLORS_YKM,
         breaks=breaks_zdwui,
         annotation_row=df_row_annots,
...
We'll exlcude 2, 5, and 16 as noisy.

## Establish Signal Genes
```{r select_signal_genes}
cluster_null_genes <- c(2,5,16)
