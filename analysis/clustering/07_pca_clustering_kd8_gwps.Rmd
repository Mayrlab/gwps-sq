---
title: "PCA-Filtered Clustering on KD8 GWPS Data"
...
## output files
FILE_OUT_TARGETS="tbl/df_target_clusters_hc_kd8_gwps_ui10.csv"
FILE_OUT_GENES="tbl/df_gene_clusters_hc_kd8_gwps_ui10.csv"
...
pheatmap(convert_rownames(mat_pca[,1:10], sgid2gene), 
         ...
         cluster_rows=TRUE, cluster_cols=TRUE, 
         cutree_rows=50,
         filename="img/heatmap-kd8-gwps-pca10-k50.pdf", width=4, height=24)
```
...
clusters_targets <- cutree(hc_targets, k=50)
...
## Z-DWUI Plot
### Possible Noise Clusters Only
We suspect the largest two cluster are mostly noise, so we want to check this in the 
DWUI representation.

#### Cluster 1
```{r plt_noise_target_cluster1_zdwui, fig.width=8, fig.height=8}
idx_cluster <- clusters_targets %>% `[`(. == 1) %>% names()
...
Yes, these both look like white noise.

### All Other Clusters

```{r plt_signal_target_clusters_zdwui, fig.width=8, fig.height=8}
idx_cluster <- clusters_targets %>% `[`(!(. %in% 1:2)) %>% names()
pheatmap(t(zdwui_gene_target[,idx_cluster]), 
         color=COLORS_BWR,
         breaks=breaks_zdwui,
         fontsize_col=1, fontsize_row=1,
...
## Reduced PCA
```{r pc_targets}
idx_signal_targets <- clusters_targets %>% `[`(!(. %in% 1:2)) %>% names()
...
clusters_genes <- cutree(hc_genes, k=20)
...
# Final Joint Plot
```{r joint_plot, figure.width=12, fig.height=24}
...
pheatmap(t(zdwui_gene_target[idx_signal_genes, idx_signal_targets]), 
         ...
         cluster_rows=TRUE, cluster_cols=TRUE, clustering_method="complete",
         filename="img/heatmap-kd6-essential-clusters-labeled.pdf", width=12, height=24)
```
