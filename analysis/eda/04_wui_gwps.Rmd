...
FILE_OUT="tbl/kd8_gwps_wui.xlsx"
DIR_IMG_OUT="img/kd8-gwps-"
...
se_tx_target <- readRDS("data/se/kd8_gwps_bulk_expressed.Rds")
```
...
```{r plt_pair_wui_etx_all, fig.width=8, fig.height=8}
g <- df_targets %>%
...
```{r static_plot_wui_etx_all, fig.width=8, fig.height=6}
df_targets %>%
    ggplot(aes(x=mean_wui, y=etx_all, color=gene_set, text=target_gene)) +
...
```{r plt_pair_etx_all_multi, fig.width=8, fig.height=8}
# g <- df_targets %>%
...
```{r static_plot_etx_all_multi, fig.width=8, fig.height=6}
df_targets %>%
    ggplot(aes(x=etx_all, y=etx_multi, color=gene_set, text=target_gene)) +
    geom_point(data=filter(df_targets, gene_set == 'other'), size=0.5) +
    geom_point(data=filter(df_targets, gene_set == 'non-targeting'), size=0.5) +
