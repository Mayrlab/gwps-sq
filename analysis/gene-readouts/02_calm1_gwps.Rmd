...
gene CALM1 as having one of the highest dynamic ranges of isoform usage accompanied by 
robust gene expression in almost all cell types. In this exploratory analysis, we
use abundance of CALM1 isoforms as a readout for shifts in isoform usage.
...
LBL_TXS=c("ENST00000356978.9"="cts_lu", 
          "ENST00000356978.9-UTR-2717"="cts_mu1",
          "ENST00000544280.6"="cts_mu2", 
          "ENST00000659177.1"="cts_su")
...
M_cell_target <- df_cells %>% 
    dplyr::select(cell_id, sgID_AB) %>% deframe() %>%
    fac2sparse() %>% t() %>%
...
g <- df_targets %>%
    ggplot(aes(x=tpm_su, y=tpm_mu1, color=gene_set, text=target_gene)) +
...
```{r static_plot_su_mu1, fig.width=8, fig.height=6}
df_targets %>%
    ggplot(aes(x=tpm_su, y=tpm_mu1, color=gene_set, text=target_gene)) +
...
ggsave(str_c(DIR_IMG_OUT, "su-mu1.pdf"), width=8, height=6, dpi=300)
```

### `r GENE` SU vs MU2
```{r plt_pair_su_mu2, fig.width=8, fig.height=8}
g <- df_targets %>%
    ggplot(aes(x=tpm_su, y=tpm_mu2, color=gene_set, text=target_gene)) +
    geom_point(aes(size=n_cells), pch=16) +
    geom_rug(size=0.1) +
    scale_color_manual(values=COLOR_MAP) +
    scale_size_continuous(range=c(0.05, 3)) +
...
```{r static_plot_su_mu2, fig.width=8, fig.height=6}
df_targets %>%
    ggplot(aes(x=tpm_su, y=tpm_mu2, color=gene_set, text=target_gene)) +
    geom_point(data=filter(df_targets, gene_set == 'other'), size=0.5) +
    geom_point(data=filter(df_targets, gene_set == 'non-targeting'), size=0.5) +
...
ggsave(str_c(DIR_IMG_OUT, "su-mu2.pdf"), width=8, height=6, dpi=300)
```

### `r GENE` MU1 vs MU2
```{r plt_pair_mu1_mu2, fig.width=8, fig.height=8}
g <- df_targets %>%
    ggplot(aes(x=tpm_mu1, y=tpm_mu2, color=gene_set, text=target_gene)) +
    geom_point(aes(size=n_cells), pch=16) +
    geom_rug(size=0.1) +
    scale_color_manual(values=COLOR_MAP) +
    scale_size_continuous(range=c(0.05, 3)) +
...
```{r static_plot_mu1_mu2, fig.width=8, fig.height=6}
df_targets %>%
    ggplot(aes(x=tpm_mu1, y=tpm_mu2, color=gene_set, text=target_gene)) +
    geom_point(data=filter(df_targets, gene_set == 'other'), size=0.5) +
    geom_point(data=filter(df_targets, gene_set == 'non-targeting'), size=0.5) +
...
ggsave(str_c(DIR_IMG_OUT, "mu1-mu2.pdf"), width=8, height=6, dpi=300)
```
...
```{r static_plot_su_lu, fig.width=8, fig.height=6}
df_targets %>%
    ggplot(aes(x=tpm_su, y=tpm_lu, color=gene_set, text=target_gene)) +
    geom_point(data=filter(df_targets, gene_set == 'other'), size=0.5) +
    geom_point(data=filter(df_targets, gene_set == 'non-targeting'), size=0.5) +
