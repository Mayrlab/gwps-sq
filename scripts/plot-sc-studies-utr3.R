...
df_sc <- read_tsv('https://www.nxn.se/single-cell-studies/data.tsv') %>%
    select(Date, `Reported cells total`, Technique) %>%
    rename(date=Date, n_cells=`Reported cells total`, technique=Technique) %>%
    mutate(date=ymd(date))

count(df_sc, technique) %>% print(n=Inf)

UTR3_COMPAT=c("BAT-Seq", "CEL-seq", "Chromium", "CITE-seq",
              "Drop-seq", "inDrops", "MARS-seq", "Microwell-seq",
              "Perturb", "sci-", "scifi-", "SCRB-seq")
UTR3_REGEX=str_c("(", str_c(UTR3_COMPAT, collapse="|"), ")")
MIN_DATE=ymd("20100101")

df_studies <- df_sc %>%
    filter(!is.na(n_cells), !is.na(technique),
           date >= MIN_DATE) %>%
    arrange(date) %>%
    mutate(utr3=str_detect(technique, UTR3_REGEX),
           n_experiments_total=row_number(),
           n_experiments_utr3=cumsum(utr3),
           n_cells_total=cumsum(n_cells),
           n_cells_utr3=cumsum(ifelse(utr3, n_cells, 0)))

df_studies %>%
    ggplot(aes(x=date)) +
    #geom_line(aes(y=n_experiments_total), color='#333333') +
    geom_ribbon(aes(ymax=n_experiments_total), ymin=0, fill='black', alpha=0.1, color='#333333', size=0.5) +
    #geom_ribbon(aes(ymax=n_experiments_utr3), ymin=0, fill='chartreuse3', alpha=0.6, color='#333333', size=0.5) +
    #geom_hline(yintercept=0, size=2) +
    scale_x_date(expand=c(0,0), limits=c(ymd(20120101), ymd(20221201))) +
    scale_y_continuous(expand=c(0,0,0.05,0), limits=c(0,NA)) +
    labs(x="Date", y="scRNA-seq studies") +
    cowplot::theme_minimal_hgrid()

ggsave("img/sc-studies/n-studies-2012-2022-total.pdf", width=3.5, height=4, dpi=300)
...
ggsave("img/sc-studies/n-studies-2012-2022-utr3.pdf", width=3.5, height=4, dpi=300)

df_studies %>%
    ggplot(aes(x=date)) +
    geom_line(aes(y=n_cells_total)) +
    geom_ribbon(aes(ymax=n_cells_utr3), ymin=0, color='#333333', fill='purple4', alpha=0.4) +
    scale_x_date(expand=c(0,1), limits=c(ymd(20120101), ymd(20221201))) +
    scale_y_continuous(expand=c(0,0,0.05,0)) +
    #scale_y_log10() +
    labs(x="Date", y="Cells sequenced") +
    cowplot::theme_minimal_hgrid()

df_studies %>%
    ggplot(aes(x=date)) +
    geom_histogram(aes(weight=n_cells, fill=utr3), position="dodge", binwidth=30, color='black', size=0.05) +
    #scale_x_date(expand=c(0,1)) +
    #scale_y_log10(limits=c(10,1e7)) +
    labs(x="Date", y="Cells sequenced per month")
    #scale_y_continuous(expand=c(0,0,0.05,0)) +
    #cowplot::theme_minimal_hgrid()

df_studies %>%
    mutate(utr3=factor(utr3, levels=c(TRUE, FALSE))) %>%
    ggplot(aes(x=date)) +
    geom_histogram(aes(weight=n_cells, fill=utr3), position="stack",
                   binwidth=30, color='black', size=0.1, alpha=0.8) +
    scale_x_date(expand=c(0,1), limits=c(ymd(20120101), ymd(20221201))) +
    scale_fill_manual(values=c("chartreuse3", "grey65"), guide=NULL) +
    #scale_y_log10() +
    labs(x="Date", y="Cells sequenced per month") +
    scale_y_continuous(expand=c(0,0,0.05,0), labels=function (breaks) { sprintf("%d M", breaks/1e6)}) +
    cowplot::theme_minimal_hgrid()

ggsave("img/sc-studies/n-cells-per-month-2012-2022.pdf", width=3.5, height=4, dpi=300)

c("white", "aliceblue", "antiquewhite", "antiquewhite1", "antiquewhite2",
